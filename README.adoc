...
CDI can also provide another scope using the `@RequestScoped` annotation to indicate that a bean is to be initialized once for every request. 
In other words, the bean is instantiated when the request is received and destroyed when a response is sent back to the client. 
A request scope is short-lived and is therefore ideal for HTTP requests.
