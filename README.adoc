...
Add the [hotspot=ApplicationScoped file=1]`@ApplicationScoped` annotation on the class to indicate that this bean is
to be initialized once for the duration of the application, where the state of the bean is shared between all the requests.

If you want this bean to be initialized once for every request, you can add the `@RequestScoped` annotation on the class instead. In other words, the bean is instantiated when the request is received and destroyed when a response is sent back to the client. A request scope is short-lived.
...
To force these test cases to run in a particular order, add the [hotspot=TestMethodOrder file=0]`@TestMethodOrder` annotation to your test class
and specify a [hotspot=MethodOrderer hotspot=TestMethodOrder file=0]`MethodOrderer` class. 
[hotspot=MethodOrderer hotspot=TestMethodOrder file=0]`OrderAnnotation.class` executes test methods in numerical order, 
...
such as `OrderAnnotation.class`, `Alphanumeric.class`, and `Random.class`. Label your test cases
with a [hotspot=Test1 hotspot=Test2 hotspot=Test3 file=0]`@Test` annotation so that it automatically runs when your test class runs.
...
Tests run: 1, Failures: 0, Errors: 0, Skipped: 0, Time elapsed: 0.325 sec - in it.io.openliberty.guides.inventory.InventoryEndpointIT
...
run the tests again to see a test failure occur.
