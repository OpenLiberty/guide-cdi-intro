...
If you want this bean is to be initialized once for every request, you can add the `@RequestScoped` annotation on the class instead. In other words, the bean is instantiated when the request is received and destroyed when a response is sent back to the client. A request scope is short-lived.
