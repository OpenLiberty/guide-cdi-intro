...
The injected bean can then be invoked directly through the [hotspot=39 file=0]`manager.add(hostname, props)`
and [hotspot=46 file=0]`manager.list()` function calls  for `InventoryManager` and [hotspot=30 file=0]`systemClient.getProperties(hostname)` for `SystemClient`.
