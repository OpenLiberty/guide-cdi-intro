...
To achieve this by using CDI, you can simply add the [hotspot=11-12]`@ApplicationScoped` annotation on the class.
...
----
Create the `Inventory Reource` class file.
...
Add the [hotspot=13-14]`@RequestScoped` annotation on the class to indicate that this bean is
...
The [hotspot=17-18]`@Inject` annotation indicates a dependency injection.
...
The injected bean can then be invoked directly through the [hotspot=25-26]`manager.get(hostname)`
and [hotspot=41-42]`manager.list()` function calls.
...
The [hotspot=27-28]`@BeforeClass` annotation is placed on a method that runs before any of the test cases.
...
The [hotspot=33-34]`@Before` and [hotspot=39-40]`@After` annotations are placed on methods that run before and after every test case.
