...
// Licensed in Creative Commons Attribution-NoDerivatives
...
Point your browser to the http://localhost:9080/inventory/systems URL. This is the staring point of the `inventory`
...
Your completed class appears as it does in the following example:
...
The `@Before` and `@After` annotations are placed on methods that start before and after every test case.
...
You can find complete Javadocs for these in the following `finish` directory for reference:
...
the `src/main/java/io/openliberty/guides/inventory/InventoryResource.java` directory to something else, then
run the Maven build again. You will see a test failure occur.
