...
The [hotspot=18-27 file=0]`add()` function is for adding entries to the inventory.
The [hotspot=29-31 file=0]`list()` function is for listing all the entries currently stored in the inventory.
...
Add the [hotspot=15 file=1]`@RequestScoped` annotation on the class to indicate that this bean is
...
Refer to the [hotspot=17-48 file=0]`InventoryResource` class you created above.

The [hotspot=19 hotspot=22 file=0]`@Inject` annotation indicates a dependency injection.
...
The injected bean `InventoryManager` can then be invoked directly through the [hotspot=39 file=0]`manager.add(hostname, props)`
and [hotspot=46 file=0]`manager.list()` function calls.  The injected bean `SystemClient` can be invoked through the 
[hotspot=30 file=0]`systemClient.getProperties(hostname)` function call.
