...
[role="code_command hotspot=14-32", subs="quotes"]
----
...
This bean contains three simple functions. The [hotspot=19-22]`get()` function is for retrieving entries from calling the `system` service.
The [hotspot=24-33]`add()` function is for adding entries to the inventory.
The [hotspot=35-37]`list()` function is for listing all the entries currently stored in the inventory.

This bean must be persistent between all of the clients, which means multiple clients need to share the same instance.
To achieve this by using CDI, you can simply add the [hotspot=12]`@ApplicationScoped` annotation onto the class.
...
[role="code_command hotspot=16-44", subs="quotes"]
----
...
Add the [hotspot=14]`@RequestScoped` annotation on the class to indicate that this bean is
...
Refer to the [hotspot=16-44]`InventoryResource` class you created above.

The [hotspot=18]`@Inject` annotation indicates a dependency injection.
...
The injected bean can then be invoked directly through the [hotspot=26]`manager.get(hostname)`
and [hotspot=42]`manager.list()` function calls.
...
[role="code_command hotspot=18-159", subs="quotes"]
----
...
The [hotspot=28]`@BeforeClass` annotation is placed on a method that runs before any of the test cases.
In this case, the [hotspot=29-32]`oneTimeSetup` method retrieves the port number for the Open Liberty server and builds
a base URL string that is used throughout the tests.

The [hotspot=34]`@Before` and [hotspot=40]`@After` annotations are placed on methods that run before and after every test case.
These methods are generally used to perform any setup and teardown tasks. In this case, the [hotspot=35-38]`setup` method
creates a JAX-RS client, which makes HTTP requests to the `inventory` service. This client must
also be registered with a JSON-P provider (`JsrJsonpProvider`) to process JSON resources. The [hotspot=41-43]`teardown`
method simply destroys this client instance.

See the following descriptions of the test cases:

[hotspot=53-65]`testEmptyInventory()` verifies that the inventory is initially empty when the server first starts up.

[hotspot=67-87]`testHostRegistration()` verifies that a host is correctly added to the inventory.

[hotspot=89-115]`testSystemPropertiesMatch()` verifies that the JVM system properties returned by the `system` service match
the ones stored in the `inventory` service.

[hotspot=117-132]`testUnknownHost()` verifies that an unknown host or a host that does not expose their JVM system
properties is correctly handled as an error.

To force these test cases to run in a particular order, put them in a [hotspot=46-51]`testSuite()` method and label
it with a [hotspot=45]`@Test` annotation so that it automatically runs when your test class run.
...
[role='command']
----
include::{common-includes}/mvnverify.adoc[][tags=!copyright]
----
