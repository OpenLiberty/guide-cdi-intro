...
You will use CDI to inject dependencies into the application and you will learn how to manage the life cycles of your objects.
...
You will use scopes and contexts in this application to manage the life cycles of your objects.
You will also inject a dependency from one class into another class.

=== Managing scopes and contexts

Open `src/main/java/io/openliberty/guides/inventory/InventoryManager.java`:
...
Add the `@ApplicationScoped` annotation on the class. This annotation is from the CDI API, and indicates that this particular
...
Next, open `src/main/java/io/openliberty/guides/inventory/InventoryResource.java`:
...
Add the `@Inject` annotation on the class:

[source, Java]
----
include::finish/src/main/java/io/openliberty/guides/inventory/InventoryResource.java[tags=Inject;!copyright]
----
...
available without the need of instantiating it yourself. The `manager.get(hostname)`
and `manager.list()` functions are called directly in this class with the @Inject declaration.
